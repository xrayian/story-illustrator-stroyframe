/**
 * Kimi K3 via Modal proxy — OpenAI-compatible backup for story analysis
 * when Gemini Pro is 429/quota. Uses the endpoint:
 *   baseURL: https://xrayian--ep-kimi-k3-server.us-west.modal.direct/v1
 *   apiKey:  `${MODAL_PROXY_TOKEN_ID}.${MODAL_PROXY_TOKEN_SECRET}`
 */

export const KIMI_DEFAULT_BASE_URL =
  "https://xrayian--ep-kimi-k3-server.us-west.modal.direct/v1";
...
async function kimiFetch(
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      const err = new Error(`Kimi ${path} failed (${res.status}): ${body.slice(0, 500)}`);
  ...
  throw new Error(`Kimi request failed after ${retries + 1} attempts: ${String(lastError)}`);
}

/**
 * Calls an OpenAI-compatible chat completions endpoint with JSON-mode
 * and parses the result. The Kimi endpoint is expected to support
 * `response_format: {type:"json_object"}`.
 */
export async function generateStructuredJsonWithKimi(
  opts: KimiStructuredOptions,
  input: string,
  retries = 2
): Promise<unknown> {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (data.error) throw new Error(`Kimi API error: ${JSON.stringify(data.error).slice(0, 500)}`);
  ...
  throw new Error(`Kimi structured generation failed after ${retries + 1} attempt(s): ${String(lastError)}`);
}
